...
/**
 * Write a C program that receives any number of strings as command line arguments. The program
 * creates two child processes, which inherit the parent's command line arguments (ie. no need to
 * send the arguments via pipe/fifo to the children for this problem). Each child process creates
 * a thread for each of the command line arguments. Each thread created by the first child will
 * extract the vowels from its argument and will append them to a string shared among the threads.
 * Each thread created by the second child process will extract the digits from its argument and will
 * add them to a sum shared among the threads. Both child processes wait for their respective threads
 * to finish and send the result to the parent via pipe. The parent displays the results.
 */
