 ...
 * Process A reads N integer numbers from the keyboard and sends them another
 * process named B. Process B will add a random number, between 2 and 5, to each
 * received number from process A and will send them to another process named C.
 * The process C will add all the received numbers and will send the result back
 * to process A. All processes will print a debug message before sending and after
 * receiving a number.
 */
